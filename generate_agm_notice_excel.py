...
① 이사 임기 유연화
   정의: 이사 임기가 특정 연수로 고정되어 있다가, 상한(최대치)만 두고 그 범위 내에서
         주총·이사회 등이 유연하게 결정할 수 있도록 바뀐 경우.
   핵심: 고정 연수 표현("N년으로 한다") → 상한+유연결정 표현("N년을 초과하지 못한다" / "N년 이내로 주주총회에서 정한다")
   ✅ 해당 예시:
     - "이사의 임기는 2년으로 한다" → "이사의 임기는 3년을 초과하지 않는 범위 내에서 주주총회에서 결정한다"
       (숫자도 늘고 유연화도 됨 → 이사 임기 연장 + 이사 임기 유연화 둘 다)
     - "이사의 임기는 3년으로 한다" → "이사의 임기는 3년을 초과하지 못한다"
       (숫자는 같고 고정→상한으로 표현만 변경 → 이사 임기 유연화만, 이사 임기 연장 아님)
   ❌ 비해당 예시:
     - "취임 후 2년 내" → "취임 후 3년 내": 숫자가 늘었지만 여전히 고정 → 이사 임기 유연화 아님 (→ 이사 임기 연장)

② 이사 임기 연장
   정의: 이사 임기의 연도 수(숫자) 자체가 늘어난 경우.
   핵심: 임기 연도 숫자의 실질적 증가 (1년→2년, 2년→3년 등)
   ✅ 해당 예시:
     - "이사의 임기는 2년으로 한다" → "이사의 임기는 3년으로 한다" (2→3, 연장)
     - "취임 후 2년 내에 종료하는..." → "취임 후 3년 내에 종료하는..." (2→3, 연장)
   ❌ 비해당 예시:
     - "이사의 임기는 3년으로 한다" → "이사의 임기는 3년을 초과하지 못한다"
       (3→3, 숫자 동일, 표현 방식만 변경 → 이사 임기 연장 아님, 이사 임기 유연화만)
     - 부칙의 시행일 날짜 변경(예: 2025년→2026년): 이사 임기 숫자와 무관

③ 이사 정원 축소
   정의: 이사 정원의 상한(최대 인원 수)이 실제로 감소한 경우.
   핵심: 상한 인원 수의 실질적 감소 (13명→7명, 16명→9명 등)
   ✅ 해당 예시:
     - "이사는 3명 이상 13명 이내" → "이사는 3명 이상 7명 이내" (13→7)
     - "이사는 3명 이상 16명 이내" → "이사는 3명 이상 9인 이하" (16→9)
   ❌ 비해당 예시:
     - "사외이사" → "독립이사"로 명칭만 변경 (인원 수 변화 없음)
     - 이사 구성 비율(사외이사 비율, 독립이사 비율) 변경 (인원 수 상한 변화 없음)
     - 감사위원회 위원 구성·선임 방식 변경
     - 정원이 그대로이거나 늘어난 경우

④ 자사주 보유
   정의: 자기주식(자사주)의 보유 또는 처분에 관한 조항이 신설되거나 변경된 경우.
   핵심: "자기주식" 또는 "자사주" 키워드 + 보유·처분 관련 조항
   ✅ 해당 예시:
     - 제9조의2(자기주식의 보유 또는 처분) 조항 신설
     - "회사는 ...자기주식을 보유·처분할 수 있다" 조항 추가
   ❌ 비해당 예시:
     - "주식의 소각" 관련 조항 신설·변경·삭제 (소각은 보유·처분과 다름)
     - 의결권 대리행사를 서면에서 전자문서로 추가하는 조항
     - 전자주주총회 개최 관련 조항

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[반드시 "기타"인 경우 — 아무리 이사·주식 관련처럼 보여도 아래는 기타]

다음 유형의 변경은 위 4가지 카테고리에 해당하지 않으므로 반드시 "기타":
  ▸ 집중투표제 배제 조항 삭제 또는 신설 (예: "집중투표제를 적용하지 아니한다" 삭제·추가)
  ▸ 사외이사 → 독립이사 명칭 변경 (단순 용어 교체, 실질 내용 변경 없음)
  ▸ 전자주주총회 도입 (원격 참석 방식 추가)
  ▸ 의결권 대리행사 방법 변경 (서면 → 서면+전자문서 등)
  ▸ 감사위원회 구성·선임 방식 변경 (분리선임 인원 변경, 의결권 제한 기준 변경 등)
  ▸ 이사회 의장 선임 방식 변경 (예: "이사회 의장을 독립이사 중에서 선임")
  ▸ 부칙 시행일 변경 (연도·날짜만 바뀌는 경우)
  ▸ 주식 소각 조항 신설·변경·삭제
  ▸ 그 외 상법 개정 반영 명칭 변경, 공고방법 변경, 주주명부 폐쇄 기준일 변경 등
...
예시4) 임기 유연화만 (숫자 동일, 표현만 변경)
  변경전: 이사 및 사외이사의 임기는 3년으로 한다
  변경후: 이사의 임기는 3년을 초과하지 못한다
  → {{"category2": "이사 임기 유연화"}}
  (※ 3년→3년으로 숫자 동일이므로 이사 임기 연장 아님)

예시5) 집중투표제 배제 조항 삭제 → 기타
  변경전: 이 회사는 이사 선임 시 집중투표제를 적용하지 아니한다
  변경후: <삭제>
  → {{"category2": "기타"}}

예시6) 전자주주총회 도입 → 기타
  변경전: 제25조(소집지) 주주총회는 본점소재지에서 개최한다
  변경후: 제25조(소집지와 개최방식) ① 현행과 동일 ② 회사는 전자적 방법에 의하여 결의에 참석할 수 있는 방식의 총회를 개최한다
  → {{"category2": "기타"}}
