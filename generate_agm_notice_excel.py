...
MAX_NOTICE_WORKERS = min(6, max(2, (os.cpu_count() or 2)))
MAX_RESULT_WORKERS = min(8, max(2, (os.cpu_count() or 2)))
...
def process_notice_report(report):
    ...
    if not folder_name:
        return {"rows": [], "fail": [corp_name, stock_code, rcept_no, "download_fail"]}

    try:
        main_file = find_main_file(folder_name)
        if not main_file:
            return {"rows": [], "fail": [corp_name, stock_code, rcept_no, "main_file_not_found"]}
        ...
        if "임시주주총회" in check_area:
            return {"rows": [], "fail": [corp_name, stock_code, rcept_no, "임시주주총회_제외"]}
        ...
        if not agenda_items:
            ...
            return {"rows": [], "fail": [corp_name, stock_code, rcept_no, reason]}
            ...
            if category1 == "정관변경":
                ...
                if not tbl and "-" in num_clean:
                    ...
                    for depth in range(len(parts) - 1, 0, -1):
                        ...
                        if candidate and ancestor not in used_charter_keys:
                            tbl = candidate
                            num_clean = ancestor
                            break
                ...
                if num_clean:
                    used_charter_keys.add(num_clean)
        ...
        return {"rows": [], "fail": [corp_name, stock_code, rcept_no, f"extract_error: {str(e)}"]}
    finally:
        shutil.rmtree(folder_name, ignore_errors=True)


def fetch_result_table_for_corp(corp_code_r, corp_name_r, result_start_date, result_end_date):
    ...
    if not rfolder:
        return corp_code_r, {}
    try:
        table = extract_agm_result_table(rfolder)
        if not table:
            print(f"  [결과표 파싱 실패] {corp_name_r}")
        return corp_code_r, table or {}
    finally:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_NOTICE_WORKERS) as executor:
        future_map = {executor.submit(process_notice_report, report): report for report in reports}
        completed = 0
        for future in as_completed(future_map):
            ...
            write_progress(progress_file, "running", percent,
                           f"{completed}/{total_reports} 처리 중: {corp_name}", completed, total_reports)
            try:
                result = future.result()
                if result.get("rows"):
                    all_rows.extend(result["rows"])
                if result.get("fail"):
                    fail_list.append(result["fail"])
            except Exception as e:
                fail_list.append([
                    corp_name,
                    str(report.get("stock_code", "")),
    ...
    with ThreadPoolExecutor(max_workers=MAX_RESULT_WORKERS) as executor:
        future_map = {
            executor.submit(
                fetch_result_table_for_corp,
                corp_code_r,
                corp_name_r,
                result_start_date,
                result_end_date
            ): (corp_code_r, corp_name_r)
            for corp_code_r, corp_name_r in seen_corp_codes.items()
        }
        for future in as_completed(future_map):
            corp_code_r, corp_name_r = future_map[future]
            try:
                cc, table = future.result()
                corp_result_map[cc] = table
            except Exception as e:
                print(f"  [결과표 수집 실패] {corp_name_r}: {e}")
