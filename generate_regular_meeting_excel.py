...
def extract_target_table_rows(file_path):
...
def main():
    ...
    for seq, row in enumerate(reports, start=1):
        ...
        try:
            table_rows = extract_target_table_rows(main_file)

            if not table_rows:
                fail_list.append([corp_name, stock_code, rcept_no, "target_table_not_found"])
                shutil.rmtree(folder_name, ignore_errors=True)
                continue
